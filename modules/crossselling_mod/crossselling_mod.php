...
class CrossSelling_Mod extends Module
{
	...
	public function hookHeader()
	{
		...
		$this->context->controller->addJS(($this->_path).'js/crossselling_mod.js');
	}
		...
		if (!$this->isCached('crossselling_mod.tpl', $this->getCacheId($cache_id)))
		{
			...
			if (count($orders))
			{
				...
				$order_products = Db::getInstance(_PS_USE_SQL_SLAVE_)->executeS(
					'
										SELECT DISTINCT od.product_id, pl.name, pl.link_rewrite, p.reference, p.available_for_order, i.id_image, product_shop.show_price, cl.link_rewrite category, p.ean13
